...
#
# quasi-newton.py: python package for steepest descent methods
#
...
def dfp(f, gradf, x0, D0=None, epsilon=1e-6, max_num_iter=1000, line_search=bisection, alpha_max=99, ls_epsilon=1e-6, ls_max_num_iter=1000):
	"""
	bfgs(): minimize f(x) using the steepest descent algorithm
	...
	1. input arguments
		...
		- D0: the initial approximation matrix for Hessian
	...
	k = 0
	xk = x0
	Dk = np.eye(len(x0)) if D0 is None else D0
	fk = f(xk)
	gk = gradf(xk)
	Dgk = Dk @ gk
	dk = -Dgk
	...
	while (np.linalg.norm(dk) > epsilon): # stopping criteria 1
		...
		dgk = gradf(xk) - gk
		Dgk = Dk @ gk
		Ck = (dxk @ dxk.T)/(dxk.T @ dgk) - (Dgk @ Dgk.T)/(dgk.T @ Dgk)
		Dk += Ck
		dk = -gradf(xk)
...
if __name__ == '__main__':
	...
	x_opt, fval_opt, status, history = dfp(f, gradf, x0, epsilon=1e-3, max_num_iter=100, line_search=bisection, alpha_max=calc_alpha_max)
